...
export const candidates: CandidateSlot[] = [
  ...
  },
  {
    ...
    shortLabel: '수 13:00',
    // 필수 참석자 일정 충돌은 확인으로 풀 수 없는 확정된 사정 → 다른 시간 권장
    recommend: 'hard',
    room: { name: '회의실 B', place: '본관 3층', capacity: '6인실' },
    avail: '필수 충돌',
    availabilityText: '김민준님 일정과 겹쳐요 · 다른 시간이 나아요',
    attendeeStatus: { kim: 'no', choi: 'avoid' },
    facts: [
      { label: '필수 참석자', value: '김민준님 일정 충돌', ok: false },
      { label: '선택 참석자', value: '최유리님 점심 직후 비선호', ok: false },
      { label: '회의실', value: '회의실 B · 본관 3층 · 6인실', ok: true },
    ],
    note: '필수 참석자의 확정된 일정과 겹쳐서, 이 시간은 바로 잡을 수 없어요',
  },
];
...
export const alternatives: CandidateSlot[] = [
  {
    ...
    recheck: [
      ...
      { id: 'choi', name: '최유리', state: 'valid', note: '점심 직후가 아니라 피하고 싶은 시간이 아니에요' },
      { id: 'jung', name: '정하늘', state: 'valid', note: '수요일 오후 캘린더가 비어 있어요' },
    ],
  },
  {
    id: 'a2',
...
export const busyBlocks: BusyBlock[] = [
  ...
  { day: 2, startHour: 13, duration: 1, label: '김민준 · 바쁨', kind: 'busy' },
  // 2명이 겹치는 시간은 나란히 표시
...
// 박서준 캘린더 기준: 목요일 외근은 아직 가능성 단계라 본인 확인이 필요하다.
export const parkBlocks: BusyBlock[] = [
  { day: 2, startHour: 10, duration: 2, label: '기술 검토', kind: 'busy' },
  { day: 3, startHour: 9, duration: 9, label: '외근 가능성', kind: 'tentative' },
  { day: 4, startHour: 9, duration: 2, label: '고객사 미팅', kind: 'busy' },
];

// 변경 후 박서준 캘린더: 외근이 화요일 오후로 확정되고, 목요일 가능성은 해소된 상태.
export const parkBlocksAfterChange: BusyBlock[] = [
  { day: 1, startHour: 13, duration: 4, label: '외근 (화요일 확정)', kind: 'out' },
  { day: 2, startHour: 10, duration: 2, label: '기술 검토', kind: 'busy' },
  { day: 4, startHour: 9, duration: 2, label: '고객사 미팅', kind: 'busy' },
];
...
// 박서준이 외근 확인 요청에 어렵다고 답한 뒤의 목요일 후보 — 불확실성이 '불가'로 풀린 상태
export const rejectedC3: CandidateSlot = {
  ...candidates[1],
  recommend: 'hard',
  avail: '참석 어려움',
  availabilityText: '박서준님이 외근이라 어렵다고 직접 답했어요',
  attendeeStatus: { park: 'no' },
  summary: '박서준님이 직접 어렵다고 답했어요 · 다른 시간이 나아요',
  facts: [
    { label: '필수 참석자', value: '박서준님 참석 어려움', ok: false },
    { label: '선택 참석자', value: '2명 캘린더상 가능', ok: true },
    { label: '회의실', value: '회의실 C · 별관 2층 · 4인실', ok: true },
  ],
  note: '박서준님이 직접 어렵다고 답해서, 이 시간은 확정하기 어려워요',
};
